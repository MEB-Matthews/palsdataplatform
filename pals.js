...
import { storeAccessToken, removeActiveClass, showAnimation, getCurrentUser, filterConsortiums, getFolderItems, filterProjects, amIViewer, getCollaboration, hideAnimation, assignNavbarActive, getFileInfo, handleRangeRequests, applicationURLs, checkDataSubmissionPermissionLevel } from './src/shared.js';
...
import {template as surveyDetailsTemplate} from './src/pages/surveyDetails.js';
...
const manageRouter = async () => {
    ...
    if(hash === '#home'){
        ...
        document.title = 'Pals Data Platform';
        ...
        if(!element) return;
        if(element.classList.contains('navbar-active')) return;
        document.title = 'Pals - Data Access';
        assignNavbarActive(element, 2);
        confluenceDiv.innerHTML = dataRequestTemplate();
    }
    else if (hash === '#surveyDetails') {
        const element = document.getElementById('surveyDetails');
        if(!element) return;
        if(element.classList.contains('navbar-active')) return;
        document.title = 'Pals - Survey Details';
        assignNavbarActive(element);
        confluenceDiv.innerHTML = surveyDetailsTemplate();
    }
    ...
    // }
        ...
        showAnimation();
        assignNavbarActive(dataDictionaryElement, 2);
        document.title = 'Pals - Data Dictionary';
        confluenceDiv.innerHTML = dataSummary('Data Dictionary', true, false, true);
        removeActiveClass('nav-link', 'active');
        try{document.querySelectorAll('[href="#data_exploration/dictionary"]')[1].classList.add('active')}
        catch(err){'Public Site'};
        dataDictionaryTemplate();
    }
...
const manageHash = async () => {
    ...
    if(hash === '#data_exploration/summary') {
        ...
        // try{document.querySelectorAll('[href="#join/description"]')[0].classList.add('navbar-active')}
        // catch(err){'Public Site'};
        document.title = 'Confluence - Resources Description';
